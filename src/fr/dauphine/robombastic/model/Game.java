...
import fr.dauphine.robombastic.main.MainGame;
...
public class Game implements Serializable {
	/**
	 * Serial Version UID (Genrated Dynamically)
	 */
	private static final long serialVersionUID = 3087260554576136310L;
	...
	/**
	 * Ce map parmet de lier le nom du jar avec la nom de la classe (nom
	 * complets) Ceci permet de recharger le même jar deux fois, et garder la
	 * trace des jars déjà charger dans le jeu
	 */
	 private Map<String, String> mapJarsBotClasses = new HashMap<String, String>();

	/**
	 * Ce map permet de faire correspondre le nom de la classe (qui est le nom
	 * de l'armée) avec la liste des bots qui constituent cette armée
	 */
	private Map<String, List<Bot>> mapNamesArmies = new HashMap<String, List<Bot>>();

	/**
	 * Ce map permet d'avoir l'acces au botContext de d'un autre Bot afin de
	 * verifier ou de modifier son état... (????)
	 */
	 ...
	 * @param bombDuration
	 */
	private void initGeneralInfo(int radarSize, int turnDuration,
			int bombDuration) {
		arenaTxt = FileUtils.readTextFromFile(this.arena);

		if (arenaTxt == null) {
			System.err
					.println("Erreur du fichier de description de l'arene du jeu !!!");
			System.exit(-1);
		}
		...
		this.generalInfo = new ImpGeneralInfo(arenaWidth, arenaHeight,
				radarSize, turnDuration, bombDuration);
		...
		for (int i = 0; i < this.generalInfo.getArenaHeight(); i++) {
			...
			for (int j = 0; j < this.generalInfo.getArenaWidth(); j++) {
				...
				if (cell == 'w') {
					...
					System.err.println("Caractère non pris en charge : "
							+ cell);
					System.exit(-1);
				}
			}
	 ...
	 * 
	 * Permet de charger les jars robots et de réaliser l'instanciation des
	 * robots et les stocker dans les listes correspondantes
	 */
	private void initRobots() {
		if (this.saved == false) {
			...
			for (int i = 0; i < armyLimit; i++) {
				...
				bot.init(botContext);
				((ImpCurrentBotInfo) botContext.getCurrentBotInfo())
						.setBot(bot);
			...
			/**
			 * Verifier le dossier de l'implémentation des robots (dossier
			 * contenant des jars)
			 */
			File file = new File(robotsDir);
			if (!file.exists()) {
				System.err.println("le dossier des robots 'robotsDir' "
						+ robotsDir + " est introuvable ...");
				System.exit(-1);
			}

			/**
			 * Récupérer les jars du dossier chaque jar contient une classe
			 * qui implémente les robots
			 */
			...
			/**
			 * Il faut avoir au moins deux jars différents (deux armée
			 * différentes)
			 */
			if (jars.length < 2) {
				System.err
						.println("Erreur, Nous devons avoir au moins deux fichiers JARs dans le repetoir : "
								+ robotsDir);
				System.exit(-1);
			}
			...
			try {
				...
				for (File jarFile : jars) {
					...
					Manifest mf = new JarFile(jarFile.getAbsolutePath())
							.getManifest();

					// récupérer le nom de la classe qui implémente
					// l'interface Bot ...
					String botClass = mf.getMainAttributes().getValue(
							"Bot-Class");
					...
					// Correspondre le nom du jar avec la liste des bots de la
					// même armée
					mapNamesArmies.put(armyName, new ArrayList<Bot>());

					/**
					 * Dans cette parti, on charge la classe qui implémente le
					 * robot afin de l'instancier
					 */
					...
					for (int i = 0; i < armyLimit; i++) {
						/**
						 * Instancier un bot à travers un constructeur par
						 * défaut (IL FAUT PAS UTILISER UN CONSTRUCTEUR AVEC
						 * PARAMETRE)
						 */
						 ...
						 */
						BotContext botContext = new ImpBotContext(this,
								botClass);
						...
						/**
						 * Ajouter le bot à la liste de son armée identifier
						 * par armyName (le nom du fichier jar)
						 */
	...
	/**
	 * Méthode utilitaire qui permt de charger une classe dynamiquement à
	 * partir d'un fichier jar
	 * 
	 * @param jarFile
	 * @param className
	 * @return
	...
	public void start() {
		...
		if (this.isPaused()) {
			if (saved == true) {
				System.err.println("SAVED START");
				resumeLastSavedGame();
			}
			System.err.println("PAUSED START");
			this.etat = Etat.started;
		} else {
			System.err.println("NORMAL START");
			this.etat = Etat.started;

			// this.initScene();
			this.initRobots();
			this.createRobotThreads(false);
	...
	public void resumeLastSavedGame() {
		System.err.print(" RESUMELASTSAVED GAME MOTHER FUCKER !!!");
		this.saved = false;
		this.etat = Etat.started;
		
		this.createRobotThreads(true);
		threadGame = new ThreadGame(this);
		threadGame.start();
	}

	/**
	 * Pour créer les threads des robots (pour chaque robot, on lui crée un
	 * thread)
	 */
	private void createRobotThreads(boolean saved) {
	 ...
	 * 
	 */
	private static class RobotThread extends Thread implements Serializable{
		/**
		 * 
		 */
		private static final long serialVersionUID = -4398958052669116092L;
	 ...
	 * @author Anis TLILANE
	 */
	private static class ThreadGame extends Thread implements Serializable{
		/**
		 * 
		 */
		private static final long serialVersionUID = 5038950238319275664L;
			...
			try {
				...
				while (Thread.currentThread().isAlive()) {// La boucle du jeu
					...
					if (game.isStarted()) {// Le cas du Démarrage
						...
						while (time + game.generalInfo.getTourTime() > (int) System
								.currentTimeMillis())
							;
					...
					// Si le jeu est achevé
						...
						game.pause();
						sleep(1000);
						System.err.println("SERIALISATION ____________");
		...
		public void serialiseGame() {
			try {
				System.err.println(game.getMapNamesArmies() + " V1");
				game.saved = true;
				SerializationUtil.serialize(this.game,
						MainGame.getSavedGamesPathFile());
				
				JOptionPane.showMessageDialog(GameFrame.getInstance(game)
						.getpInfo(), "your game was saved.",
		...
		public void endGame() throws DeadBotException {
			...
			if (nbBotAliveR1 == 0) {
				game.pause();
				JOptionPane.showMessageDialog(GameFrame.getInstance(game)
						.getpInfo(), "Vainqueur : Armées Robot02",
						"Information", JOptionPane.INFORMATION_MESSAGE);
				System.exit(0);
			} else {
				int nbBotAliveR2 = 0;
				for (Bot bot : game.mapNamesArmies.get("Robot02")) {
					if (((Robot) bot).getBotContext().getCurrentBotInfo()
							.isAlive()) {
						nbBotAliveR2++;
					}
				}
				if (nbBotAliveR2 == 0) {
					game.pause();
					JOptionPane.showMessageDialog(GameFrame.getInstance(game)
							.getpInfo(), "Vainqueur : Armées Robot01",
							"Information", JOptionPane.INFORMATION_MESSAGE);
					System.exit(0);
				}
			}
